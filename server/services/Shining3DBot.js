...
async function loginAndFetchShiningCases(username, password) {
    ...
    const browser = await puppeteer.launch({
        // headless: 'new' is preferred for modern Puppeteer, or use true
        headless: 'new', // ✅ FIX: Must be headless on a server
        args: [
            '--no-sandbox',
            '--disable-setuid-sandbox',
            '--disable-gpu', // Recommended for headless on servers
            '--no-zygote', // Recommended for some server environments
            '--no-first-run',
        ...
        // ✅ FIX: Specify executablePath for Render's Chromium
        executablePath: process.env.CHROMIUM_PATH || '/usr/bin/chromium-browser', // Common path on Render
        // Note: 'ignoreDefaultArgs' with '--enable-automation' might be problematic with executablePath,
        // it's often more reliable to let Puppeteer manage arguments once executablePath is set.
        // If issues persist, try removing ignoreDefaultArgs.
...
async function downloadShining3DCase(caseId) {
    ...
    const browser = await puppeteer.launch({
        ...
        args: [
            ...
            '--start-maximized', // start Chrome maximized (only effective in non-headless)
            '--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36',
        ],
        defaultViewport: null, // important to use full window size (only effective in non-headless)
        // ✅ FIX: Specify executablePath for Render's Chromium
        executablePath: process.env.CHROMIUM_PATH || '/usr/bin/chromium-browser', // Common path on Render
        ignoreDefaultArgs: ['--enable-automation'],
    });
